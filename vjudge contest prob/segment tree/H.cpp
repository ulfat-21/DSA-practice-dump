...
Node mergeNode(Node a,Node b)
{
    ...
    ans.suff=max(b.suff,b.sum+a.suff);///a er jaygay b, pref er jaygay sum interchange
    ans.res=max(a.res,b.res);///
    ans.res=max(ans.res,a.suff+b.pref);///
    return ans;
}
...
void buildTree(vector<long long>& arr,vector<Node>& st,int node,int tL,int tR)
{
    if(tL==tR)
    {
        ...
        st[node].suff = max(0LL, arr[tL]);///
        st[node].res = max(0LL, arr[tL]);///
        return;
    }
    int mid = (tL + tR) / 2;
...
void pointUpdate(vector<Node>& st,int node,int tL,int tR,int idx,long long val)
{
    if(tL==tR)
    {
        ...
        st[node].suff = max(0LL, val);///
        st[node].res = max(0LL, val);///
        return;
    }
    int mid = (tL + tR) / 2;
    int left_child = 2 * node;
